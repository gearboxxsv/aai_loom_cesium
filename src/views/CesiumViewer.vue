<template>
 ...
 -->
    ...
    <div>
      ...
      <v-btn color="blue" v-on:click="getPosition">GET POSITION</v-btn>
    </div>
  -->
  </v-container>
</template>
  ...
  methods: {
    ...
    getPosition() {
      if (viewer2.scene.mode == 3) {
        var windowPosition = new Cesium.Cartesian2(
          viewer2.container.clientWidth / 2,
          viewer2.container.clientHeight / 2
        )
        var pickRay = viewer2.scene.camera.getPickRay(windowPosition)
        var pickPosition = viewer2.scene.globe.pick(pickRay, viewer2.scene)
        var pickPositionCartographic =
          viewer2.scene.globe.ellipsoid.cartesianToCartographic(pickPosition)
        console.log(pickPositionCartographic.longitude * (180 / Math.PI))
        console.log(pickPositionCartographic.latitude * (180 / Math.PI))
      } else if (viewer2.scene.mode == 2) {
        var camPos = viewer2.camera.positionCartographic
        console.log(
    ...
    addVehicle(data) {
      ...
      */
      ...
      const vehicleEntity = viewer2.entities.add({
        id: data.hex,
    ...
    updateVehicle(data) {
      ...
      const vehicleToUpdate = viewer2.entities.getById(data.prev.hex)
    ...
    removeVehicle(data) {
      viewer2.entities.remove(viewer2.entities.getById(data.hex))
  ...
  mounted() {
    server.connect().then(() => {
      server.call('initCesiumGPS').then((data) => {
        ...
        let initViewerBounds = viewer2.entities.add({
        ...
        viewer2.zoomTo(initViewerBounds)
        ...
        subAirwareness.ready().then(() => {
          ...
          for (var i = 0; i < dataAirwareness.length; i++) {
            ...
            this.addVehicle(dataAirwareness[i])
          }
          ...
          server.collection('streamer').onChange((data) => {
            ...
            if (data.changed) {
              this.updateVehicle(data.changed)
            }
            if (data.added) {
              this.addVehicle(data.added)
            }
            if (data.removed) {
              this.removeVehicle(data.removed)
            }
          })
        })
      })
